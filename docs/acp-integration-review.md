...
- Workspace and chat reordering starts after a small mouse-drag threshold, or a 500 ms touch hold. Touch movement before the hold cancels it so scrolling still works. Pointer movement and release are observed at the root to handle ListBox/Button capture; workspace drops use the heading midpoint, and an insertion line shows the destination. Escape cancels a drag.
...
September 15 VT Code authentication correction: a real prompt reproduced a 401 after switching from OpenRouter to OpenAI through ACP. VT Code 0.162.4 loads its ChatGPT OAuth handle only for the startup provider; changing the ACP provider does not reload it. When the host reports OpenAI credentials, the default VT Code launch now selects OpenAI with its standard authentication environment, then reapplies the ACP provider to normalize any inherited incompatible model. Explicit custom provider/credential commands remain untouched. A live ChatRuntime test in Debian WSL completed a diagnostic prompt using the existing ChatGPT login. Successful prompts clear the cached sign-in warning. ACP plan notifications now update one collapsible plan per turn, separate from assistant text.
