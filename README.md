...
Use `vibe-harder .` to open the current directory, or `vibe-harder "path to a folder"`. An existing local workspace is selected (including previously closed workspaces); otherwise one is created. The running app is reused. `vibe-harder` with no argument just brings up the app. Windows installs `vibe-harder.cmd` on your user PATH (open a new terminal); Linux installs a shell launcher at `~/.local/bin/vibe-harder`. On macOS, after moving the app to Applications, run `sh "/Applications/Vibe Harder.app/Contents/MacOS/install-cli.sh"`. Add `~/.local/bin` to PATH if your shell does not already include it.
