...
Install the tested Android workload with `dotnet workload install android --from-rollback-file packaging/android-workload.json`. This pins the same manifest used by release CI; an unpinned workload can request Native AOT runtime packages newer than those published for the SDK.
