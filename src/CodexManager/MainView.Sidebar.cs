    ...
    private void RefreshRemoteSidebar()
    {
        ...
        foreach (var (remoteHost, catalog) in remoteCatalogs)
        {
            ...
            foreach (var workspace in SidebarOrder.Apply(store, "workspaces:" + connectionScope, catalog["workspaces"]!.AsArray().OfType<JsonNode>(), w => w["id"]!.GetValue<string>()))
            {
                ...
                var create = new IconButton { Icon = "add", IconSize = 10, Label = "New chat", Classes = { "rowAction" } }; create.Click += (_, _) => view.ShowNewChat(create, ownerId, () => OpenRemoteHost(host)); Grid.SetColumn(create, 2); header.Children.Add(create);
