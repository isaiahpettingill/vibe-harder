...
public sealed class RemoteTerminals(Store store, IList<Workspace> workspaces) : IDisposable
{
    private sealed class Shell : IDisposable
    {
        ...
        public long Start;
        public DateTime Used = DateTime.UtcNow;
        public Shell() => Session.RawOutput += text =>
        {
            Buffer.Append(text);
    ...
    public async Task<JsonNode?> Handle(JsonObject request)
    {
        ...
        if (method == "terminal/open")
        {
            ...
            foreach (var stale in shells.Where(s => s.Value.Used < DateTime.UtcNow.AddMinutes(-30)).Select(s => s.Key).ToArray()) { shells[stale].Dispose(); shells.Remove(stale); }
            if (shells.Count >= 16) throw new IOException("Close an existing remote terminal before opening another.");
            var shell = new Shell(); var id = Guid.NewGuid().ToString("N"); shells.Add(id, shell);
            try { await shell.Session.Start(workspace, settings: store); }
            catch { shells.Remove(id); shell.Dispose(); throw; }
            return new JsonObject { ["id"] = id };
        }
        if (!shells.TryGetValue(Text("terminalId"), out var active)) throw new IOException("This terminal has closed. Open it again to start a new shell.");
        active.Used = DateTime.UtcNow;
        switch (method)
        {
            case "terminal/read":
                ...
                return new JsonObject { ["text"] = active.Buffer.ToString((int)(start - active.Start), (int)(end - start)), ["offset"] = end, ["reset"] = offset < active.Start || offset > end };
            case "terminal/input":
                var input = Text("text");
            ...
            case "terminal/close":
                active.Dispose(); shells.Remove(Text("terminalId")); break;
