...
public sealed class RemoteSettings : Window
{
    ...
    public RemoteSettings(Store store, string? fingerprint, Func<Task>? configureServer = null, Func<Task>? configureWeb = null)
    {
        Title = "Remote connections"; Width = 500; Height = 580; WindowStartupLocation = WindowStartupLocation.CenterOwner;
        var settings = new ConnectionSettingsView(store, false, configureServer ?? (() => Task.CompletedTask), configureWeb);
        Content = settings;
