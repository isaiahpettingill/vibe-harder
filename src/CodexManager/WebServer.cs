        ...
        app.Map("/remote", async context =>
        {
            ...
            try
            {
                ...
                await protocol.Serve((token, maximum) => WebSocketWire.Read(socket, token, maximum),
                    (value, token) => WebSocketWire.Write(socket, value, token), socket.Abort, binding, session.Token);
            }
            catch (Exception error) when (error is IOException or OperationCanceledException or System.Net.WebSockets.WebSocketException) { }
